  ...
  const viewsEnriched = useMemo(() => {
    const out = (views ?? [])
      .map((v) => ({
        ...
        title: (() => {
        const id = v.movie_id;
        const canonical = typeof id === "string" && id.startsWith("bp_") ? id.slice(3) : id;
        return movieTitleById.get(canonical) ?? nomineeById.get(id)?.title ?? id;
      })(),
      }))
    ...
    out.sort((a, b) => b.seen_count - a.seen_count);
    return out;
  }, [views, movieTitleById, nomineeById]);
